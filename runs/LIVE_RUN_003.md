...
PROTOCOL: 1.6.1 — vintage declaration (PROTOCOL §11, rule vintage). This transcript predates the pin rule (§1, added 1.13.0) and the sense floor (§11, added 1.16.0); it is linted by the rules it was written under, annotated, never retouched. (Added in the v1.17.0 design audit, which found this transcript was the only one of its cohort missing the declaration its three siblings carry.)
...
(a Flask app; the SUBJECT_B system) and proved the suite **finds bugs**. This run turns the
