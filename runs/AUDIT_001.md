...
external engineer's audit of LIVE_RUN_004 (the TickIt security review). This file records each
...
**Evidence caps stated up front.** The TickIt repo is not reachable from this environment, so the
auditor's claims *about TickIt's code* (schema comments, `ledger.ts` intent notes, the hosted
...
not TickIt's model: schema comments say "Everyone reads (analytics)", `fetchProjectLedger` is
...
same rows to non-members is incoherent product behavior. (b) Membership is decorative in TickIt —
