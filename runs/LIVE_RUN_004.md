# LIVE_RUN_004 — SUBJECT_C (the intended LIVE_RUN_001 target, finally reachable)

PROTOCOL: 1.12.0 — vintage declaration (PROTOCOL §11, rule vintage). This transcript predates the sense floor (§11, added 1.16.0) and is linted by the rules it was written under; it is annotated, never retouched.
...
**Target:** `<redacted local path>` — the **SUBJECT_C** app (`package.json` name: `SUBJECT_C`).
...
**Why this target:** LIVE_RUN_001 records verbatim that its intended target was *"`SUBJECT_C`,
private, unreachable from the run environment"* and used a Flask stand-in instead. This repo is
that `SUBJECT_C`, now reachable — running it closes a loop the suite's own history left open. It also
...
the artifact) but **not committed to the SUBJECT_C repo** — SUBJECT_C is a live, deployed project and
...
Ran SUBJECT_C's own test suite before judging anything:
...
establishes for the rest of the run: SUBJECT_C's security-relevant behavior lives in Postgres RLS +
...
SUBJECT_C's public export API has a **hole in one of its four access modes**. Three of the four modes
...
Per chief-engineer Phase 1 census: **no PROTOCOL-registry ledgers exist in the SUBJECT_C repo**
(no PROBLEM_BRIEF, ARCHITECTURE, THREAT_MODEL, etc.) — SUBJECT_C was built without the suite. Its own
...
live SUBJECT_C+Supabase with two ordinary users and one project. That environment is not reachable
...
```
$ python tools/structure-report.py "…/SUBJECT_C/src"
...
logged below as a (trace-only) suite observation, not a SUBJECT_C finding.
...
**Promotion check (turns F1 (trace-only) → (proven)):** with two users A and B and a project
owned by B that A is not a member of, request the project-scoped entries endpoint as A. Today it
returns B's project entries (the bug); after the fix it must return `403`. *(The literal request
was redacted at publication: SUBJECT_C is a live system and this finding was open when the run
was written. The check is stated in full — anyone holding the subject can run it — while the
copy-paste invocation is not.)*
...
1. **(proven) — `run-trace.py` has no completeness profile for a security-audit request.**
   ...
   **false red** (misclassified as `review`, demanding a REVIEW verdict a pure threat run correctly
   ...
   for the SUBJECT_C BOLA and the completeness checker could not account for it.
